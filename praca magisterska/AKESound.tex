...
To exchange data are used mainly radio channels like Bluetooth, IRDa, Internet. Is possible to gain control over these kind of channels and make MITMA or others attacks including eavesdropping. We want to check how sound waves (especially ultrasound) will be useful to establishing secure channel.
...
Protocol is very simple to implement. AMA is defined a cyclic group G where Computational Diffie-Hellman Problem (CDH) is hard to solve. At the beginning, parties (Alice and Bob) generate their private key - $x_A$ (respectively, $x_B$) and public key $y_A = g^{x_A}$ (respectively, $y_A$). Then starts main procedure. Parties generate their ephemeral keys. Private is $h_A := H(a)$ (where $a$ is a random number) (respectively $h_B$, $b$) and public ephemeral key $c_A := g^{h_A}$ (respectively $c_B$). In the first phase parties exchange ephemeral keys, based on Diffie-Hellman key exchange. 
...
To authentication, parties encrypt their certificates $cert_A$ and $cert_B$ and the challenge values $r_A$ and $r_B$ which are the hash of two keys 
Authenticated party is raising ephemeral public key $c_B$ (respectively, $c_A$) to power of private key $x_A$ (respectively, $x_A$). Verifier compute the same value without private key but with the discrete logarithm of $c_A$ (respectively, $c_B$) which is $y_A^{h_B}$ ($y_B^{h_A}$). If the compared values are equal then $K_{session}$ is the hash of master key $K$ and new parameter.

\vspace{5mm}

This protocol is proved to be secure under Random Oracle Model (ROM) for the used hash function $H$. Additionally is secure under Computational Diffie Hellman Assumption.
\newpage
\begin{table}[H]

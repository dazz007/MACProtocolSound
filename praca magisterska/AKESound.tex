...
Mentioned method is secure against eavesdropping, however it does not authenticate both parties to each other and thus does not guarantee security when adversary can take control over the channel in which the key establishment is being performed. Adversary taking control over the channel can perform Man-In-The-Middle-Attack (MITMA). MITMA is that, the adversary make connection with other parties trying to authenticate being not detected. He can manipulate of the communication pretending to be one of the party, changing or sending own messages, making them to believe that they are still talking to each other. The goal of these processes is obtain secrete key by attacker and thus decrypt and even modify all messages exchanged between parties.

\vspace{5mm}

Main ways to communicate between devices are e.g. Bluetooth or WiFi.
\subsection{Authenticated Key Agreement}

Authenticated Key Agreement Protocols base on public key cryptography and trusted authorities. In public key cryptography are required two separate keys where one is private and second one is public. This is the most common solution being used.
...
Beyond the focus of safety AKE protocol we wanted to study not typical communication channel to establish session key. Is worth to ensure that adversary will has as small as possible access to protocol transcript. 

\vspace{5mm}

During the secure exchange data are needed two channels for communication between two parties. First channel, to establish session keys, should be very difficult to spy and taking over control for adversary. The second channel can be insecure because established session key should ensure that content of protected data intended for one of the parties can be encrypt only by the party participated in protocol. For the key establishment in our work was chosen sound channel, second channel is wireless.
...
Is needed to consider scenario where adversary learns some secrete information from parties participate in protocol. The worst case is when adversary obtain the long term secrete key from one of the party. Having this key adversary can totally control this party. Then this party is as corrupted. This lead to belows property of the KE protocol.
...
There are a lot of techniques to transfer secret data between devices. The most popular are e.g. Bluetooth or WiFi, but no every devices has these kind of appliance. Additionally these techniques are vulnerable to attacks which are hard to detect. Participate parties in protocol are not able to say that someone is manipulating their session, hijacking messages or changing them. They do not even know that sent data reach appropriate party. 

\vspace{5mm}

Safety channel could be for example two devices connected by wire. For the third party would be very difficult to eavesdrop the session. Attacker should be connected also to this wire but it would be visible for the other parties and easily to detect him. Unfortunately, carrying additional cable every time when the parties want to establish session key is inconvenient.

\vspace{5mm}

The other possibility could be optical channel. Sharing data using e.g. QR codes is also interesting way to authentication of devices. Attacker would have difficult task to manipulate shared data if e.g. two devices are covered by some walls. However, not every device has camera with appropriate quality that can read QR codes with appropriate big public keys.

\vspace{5mm}

Another way is to use vibrations []. New smartphones have installed sensor of vibration so it is possible to create such a communication channel. Attacker to manipulate this kind of channel has to generate own vibrations and it would be easily detected by authenticating parties.

\vspace{5mm}

Sound channel is also one of the options. Every mobile device have speaker and microphone, so we do not need to worry about no available Internet or that the other party has not installed Bluetooth or IrDA. Using ultrasound, the signal will be not detected by humans ear. Additionally mobile phones cannot generate these waves with big intense, so transferring data should be done on small distance. Of course sound channel could be easily to manipulate. Attacker with proper speaker can generate frequencies which could disturb the communication. However, he needs to be close to the authenticating parties if generated ultrasound has to be detected by party's mobiles microphone or generate sound waves with big intense which could be audible for human. 

\subsection{Sound channel parameters}

Using Diffie-Hellman key exchange and asymmetric cryptography should be use the longest key length as possible. In NIST specification SP 800-131 is mentioned that recommended key length is 3072. However, in out prototype we used 1024 key length. The reason is low speed of transferring data, which is 200b/s. Decreasing time of sending one character had influence on analyzing received data. It have been taking too long time (frequency of getting data increased, so algorithm did not follow in analyze so much data in short time).
...
Lets assumed that Alice and Bob want to establish a secure connection between their mobile phones using sound channel. To be sure that every high frequency will be detected, they try to put their phone in a such way that microphone is as close as possible to speaker of the second phone and vice versa. Usually the speaker and microphone are set at the bottom of device, so mobile phones should be directed bottom to each other. Sound waves are spreading, so worth is protect against it. Good choice are boxes build in lead. Lead does not pass the waves, so attacker could have more difficult job to obtain protocol transcript. To obtain session key is processed AKE protocol. After this process further exchange of data can be done by WiFi, because of the speed of transferring data which is higher. 
...
Anonymous Mutual Chip Authentication (described in detail in section 3) should ensure that even if some transferring data eavesdrop then adversary will do nothing with this data. Duration of the protocol is about 40 second - more information about duration and performance are described in section 5.
...
To establish session keys we used in our prototype Anonymous Mutual Authentication protocol which base on Diffie-Hellman key exchange. This protocol have never been implemented on the mobile phone with system Android. 
...
Protocol is very simple to implement. AMA is defined a cyclic group G where Computational Diffie-Hellman Problem (CDH) is hard. At the beginning, parties (Alice and Bob) generate their private key - $x_A$ (respectively, $x_B$) and public key $y_A = g^{x_A}$ (respectively, $y_A$). Then starts main procedure. Parties generate their ephemeral keys. Private is $h_A := H(a)$ (where $a$ is a random number) (respectively $h_B$, $b$) and public ephemeral key $c_A := g^{h_A}$ (respectively $c_B$). In the first phase parties exchange ephemeral keys, based on Diffie-Hellman key exchange. 

When parties obtain ephemeral public key of another party, they start compute master key $K$. Four different one-time keys are establish by hashing $K$ and number parameter - different for each one-time key. 
...
The key exchange protocol have to ensure that even eavesdropping data will be not useful for eavesdropper or 
...
Sum of three functions with frequencies 3m 7m and 11 Hz on a graph:
\begin{figure}[here]
	\centering
...
Application analyzes periodically finite number of samples of sound. To converse to frequency domain is used Discrete Fourier transform (DFT). Input samples are in the form of complex numbers, but in practice used only real numbers. Output samples are complex numbers. The following graphs show continuous signal and discretized signal \cite{fourier}:
...
In our protocol is used Hanning window which is noticed by:


As one can see at \ref{fig:F10} the pitch is wider, hence is easier to determine the correct frequency.
\begin{figure}[!]
	\centering
...
\end{itemize}
We did not want to go below 10 kHz and higher frequency than 20,5 is hard to detect for mobile phone (especially for LG). Distance 100 Hz is the smallest to test because of implemented algorithm. During the tests, proved to be that for almost every test cases, protocol was successful in 70\%. Except the last one always end by failure. Character which represent end of sending data has always the highest frequency. In this case it was 20,8 kHz. Only LG Swift cannot registry that frequency. Samsung dealt with it flawlessly.

\vspace{5mm}

There are several reason why the result was not 100\%. Sometimes, if we send, for example, frequency 20 kHz then second device receive it as 19,9. It can be caused by some distortion on the microphone on even on the speaker. What happened was that the speaker make sound different than usual. Was needed restart of device. Sometimes it is sufficient that one of the device froze on some milliseconds and then receiving device lose some characters. 

\vspace{5mm}

Measure of allowed distance between devices we have started with stick microphone from LG with speaker from Samsung. This gave us the best results. Received amplitude was the biggest, hence easier, for algorithm, was to find interesting for us frequency. Then we increase distance to 2 cm. In this case results was also good. Sessions keys was established almost every time. Problem was started increasing distance to 6 cm. Bigger frequencies was not registered by microphone.
